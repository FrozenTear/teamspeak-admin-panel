...
| 6 | Define a trivial flow, enable it, observe it triggers | **cut for v1.0** | Engine never landed: no `crates/ts6-manager-server/src/flow/`, no `flows_router` merged into `main.rs`; only `crates/ts6-manager-server/src/repos/bot_flows.rs` exists, unreferenced by any HTTP surface. | **Out of v1.0 gate scope.** Deferred to v1.1 — design brief tracked under [PURA-155](/PURA/issues/PURA-155); no WS-Gate row, no operator-facing surface in `v1.0`. Decision: [PURA-195](/PURA/issues/PURA-195). |
...
**Summary read:** the v1.0 gate now runs **six rows** (V1, V2, V3, V4, V5, V7). Of those, V1/V2/V3/V4/V7 are functionally wired against the local fixture and ran green at the Phase 1/2 gates ([PURA-74](/PURA/issues/PURA-74)); V5 (music bot) is the highest-risk gate because the wire landed in `885f3f3` and has not had an audible end-to-end smoke. V6 (flow trigger) is **cut from v1.0** per [PURA-195](/PURA/issues/PURA-195) — the engine never landed in the release artefact and the deferral is an honest scope correction, not a recovery. None of the remaining six has been run against a *fresh rootless Podman deploy of a release-tagged OCI image* — that is what Phase 6 §5 calls the v1.0 gate. The audit's main finding is that the Chapter 1 verifications, taken individually, are mostly ready; the gate that *combines* them under release packaging conditions is the unbuilt artefact.
