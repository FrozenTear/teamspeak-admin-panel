...
| 4 | Issue at least one mutating action against the TS server (e.g., kick a client) | pass (against fixture) → partial (against gate deploy) | Phase 2 epic [PURA-66](/PURA/issues/PURA-66); QA evidence [PURA-74](/PURA/issues/PURA-74) ran V4 against the real TS6 fixture. | Needs WS-Gate rerun against the v1.0 image; otherwise wired. |
...
| 6 | Define a trivial flow, enable it, observe it triggers | partial | Flow engine: `crates/ts6-manager-server/src/flow/`; Phase 3 epic. | Need to confirm the trigger→action loop still works under the post-Phase 5 codebase (no regression run since `c56f84c`). |
...
**Summary read:** four verifications (V1, V2, V3, V4, V7) are functionally wired against the local fixture and ran green at the Phase 1/2 gates ([PURA-74](/PURA/issues/PURA-74)); V6 (flow trigger) has not been re-exercised since end of Phase 3; V5 (music bot) is the highest-risk gate because the wire landed yesterday and has not had an audible end-to-end smoke. None of the seven has been run against a *fresh rootless Podman deploy of a release-tagged OCI image* — that is what Phase 6 §5 calls the v1.0 gate. The audit's main finding is that the Chapter 1 verifications, taken individually, are mostly ready; the gate that *combines* them under release packaging conditions is the unbuilt artefact.
...
12. **WS-Gate** — run all seven Chapter 1 verifications against a fresh rootless Podman deploy of the v1.0 OCI image, on a real host. Tag v1.0 on green. Blocked by WS-OPS-\* and WS-Security-R6 + R8.
