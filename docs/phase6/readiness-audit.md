...
| 6g | Graph flow — fire a multi-node graph (`transform → branch → parallel(sub-flow) → join`) and assert the per-node outcome: taken branch `ok`, pruned branches `skipped`, parallel paths `ok`, run `ok` | pass (v1.2) | Graph/node flow engine shipped per [PURA-259](/PURA/issues/PURA-259) — engine: `crates/ts6-manager-server/src/flow/engine/graph.rs` ([PURA-266](/PURA/issues/PURA-266)); wire types: `crates/shared/src/flows/v2.rs` ([PURA-265](/PURA/issues/PURA-265)); REST surface: `crates/ts6-manager-server/src/flow/routes.rs` ([PURA-278](/PURA/issues/PURA-278)). Probe: `scripts/ws-gate/v6-graph-probe.sh` ([PURA-268](/PURA/issues/PURA-268)). | None. Probe ran **green against a local v2 deploy**: exit `0` with all per-node assertions (taken branch `a` = `ok`, pruned `b`/`default` = `skipped`, `parallel` fan-out = `ok`, run `ok`) plus the negative `graph_cycle` reject; exit `66` fail-fast against a v1.1-routes image. The kept `v6-probe.sh` (linear V6) also stays green on the v2 image. The release-image run against a rootless-Podman OCI artefact is the v1.2 release gate's job. |
