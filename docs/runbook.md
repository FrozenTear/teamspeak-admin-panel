...
`scripts/apply-fullstack-soft-pin.sh`). Live fullstack stays `2-5` /
`-5` until Robert picks packing A (shrink ACK) or B. Bot
`TS6_BOT_CPUSET=0-1` is an in-process send-thread pin (option 1) —
never a container-wide `0-1` cpuset (packing C; Angerfist
163/590/117). `TS6_BOT_DECODE_CPUSET` is kube env, empty until A/B.
Sidecar stays unpinned. The music kube container has no `envFrom` secrets;
